...
class basic_tests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.opts = {'verbose': False,
                     ...
                     'check_termination': 1,
                     'warm_starting': True}
        self.model = osqp.OSQP(pybind11=True)
    ...
    def test_update_max_iter(self):
        ...
        res, info = self.model.solve()

        # Assert max iter reached
        self.assertEqual(info.status_val,
                         constant('OSQP_MAX_ITER_REACHED', pybind11=True))
    ...
    def test_update_check_termination(self):
        ...
        # Assert max iter reached
        self.assertEqual(info.iter, self.opts['max_iter'])

    @mark.xfail
    def test_update_rho(self):
        res_default, res_default_info = self.model.solve()
        ...
        res_updated_rho, res_updated_rho_info = self.model.solve()

        # Assert same number of iterations
        self.assertEqual(res_default_info.iter, res_updated_rho_info.iter)
    ...
    def test_upper_triangular_P(self):
        ...
        m = osqp.OSQP(pybind11=True)
        m.setup(P=P_triu, q=self.q, A=self.A, l=self.l, u=self.u,
                **self.opts)
        res_triu, res_triu_info = m.solve()
        ...
        nptest.assert_allclose(res_default.y, res_triu.y,
                               rtol=rel_tol, atol=abs_tol)
        nptest.assert_almost_equal(res_default_info.obj_val,
                                   res_triu_info.obj_val,
                                   decimal=decimal_tol)
